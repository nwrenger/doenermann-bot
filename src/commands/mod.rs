...
pub fn load_birthdays(path: &str) -> Result<Vec<BirthdayRow>, Error> {
    let mut rdr = Reader::from_path(path).map_err(|_| Error::ReadCSV)?;
...
pub fn save_birthdays(path: &str, rows: &[BirthdayRow]) -> Result<(), Error> {
    let mut wtr = Writer::from_path(path).map_err(|_| Error::WriteCSV)?;
    for row in rows {
        wtr.serialize(row)
